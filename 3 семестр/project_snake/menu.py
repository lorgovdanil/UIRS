...
from Snake import Snake, create_apple, delete_apple, Score, delete_mega_apple, create_mega_apple, \
    delete_black_apple, create_black_apple, Score2, Timer, TableHP
from game_file_rezult import update_data, show_rezult, write_file
from setting_file import write_setting_file, read_setting_file
...
def create_game_dual(root):
    ...
    def create_snake2(color2, name2):
        return Snake(0, CELL_HEIGHT - 1, CELL_WEIGHT, CELL_HEIGHT, color2, name2, "Up", "Down", "Left", "Right", canvas,
                     POLE, SEG_SIZE)
...
class MenuWindow:
    ...
    def create_menu_widgets(self):
        ...
        duel_game_button = ttk.Button(self.menu_frame, text="Дуэль", command=self.start_duel_game, style="Menu.TButton",
                                      cursor="spider")
        duel_game_button.pack(pady=10)

        # Кнопка для игры на одного игрока
        single_game_button = ttk.Button(self.menu_frame, text="Одна змейка", command=self.start_single_game,
                                        style="Menu.TButton")
        single_game_button.pack(pady=10)
        # Кнопка для выживания
        survival_game_button = ttk.Button(self.menu_frame, text="Выживание", command=self.start_survival_game,
                                          style="Menu.TButton")
    ...
    def show_scores(self):
        ...
        # Кнопка
        delete_button = ttk.Button(self.root, text="Очистить результаты", width=40, command=self.delete_rezult,
                                   style="Menu.TButton")
